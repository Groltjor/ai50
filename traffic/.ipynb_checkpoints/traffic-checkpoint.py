...
def load_data(data_dir):
    ...
    for folder in os.listdir(dataSetPath):
        ...
        for archivo in os.listdir(folderPath):
            ...
            archivoPath = os.path.join(folderPath, archivo)
            img = cv.imread(archivoPath)
            img = cv.cvtColor(img, cv.COLOR_BGR2RGB)
            if len(img.shape) != 3:
                print(alerta*20)
            res = cv.resize(img, (imgW, imgH), interpolation = cv.INTER_LINEAR)
            imagenes.append(res)
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        tf.keras.Input(shape= (imgW, imgH, 3)),
