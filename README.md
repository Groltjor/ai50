# Repositorio de AI50 - Introduction to Artificial Intelligence with Python

Este repositorio cuenta con todas mis practicas del curso AI50, en las cuales se proporcionaba una serie de ejercicios donde mi trabajo era implementar algoritmos clásicos de Inteligencia Artifical y en los últimos ejercicios el uso intensivo de librerias como TensorFlow o NLPK para realizar tareas de aprendizaje y procesamiento de lenguaje.

## Certificado de compleción

El siguiente certificado avala que complete éxitosamente y pase las pruebas técnicas de cada uno de los proyectos listados en mi repositorio. Este para ser emitido debe cada uno de los proyectos ser entregado a través del submit50, librería para la evaluacion de la seríe de cursos del CS50.

[https://certificates.cs50.io/33b5e452-0447-4a93-a170-c7b748920041.pdf?size=letter](https://certificates.cs50.io/33b5e452-0447-4a93-a170-c7b748920041.pdf?size=letter)

## Tecnologias y enfoques aplicados en el AI50

- **Búsqueda con inteligencia artificial**
  - Algoritmos de búsqueda no informada: Depth-First Search (DFS), Breadth-First Search (BFS)
  - Búsqueda informada: A* y heurísticas
  - Modelado de problemas como grafos

- **Lógica proposicional**
  - Inferencia con satisfacibilidad (SAT solvers)
  - Conjuntos de conocimiento y modelos
  - Resolución lógica y deducción de hechos

- **Inferencia probabilística**
  - Redes bayesianas
  - Herencia genética y probabilidad condicional
  - Aplicaciones de inferencia probabilística en dominios inciertos

- **Planeación**
  - Representación de estados y acciones
  - Planificación basada en búsqueda
  - Algoritmos como GraphPlan

- **Aprendizaje automático**
  - Clasificación con perceptrones y aprendizaje supervisado
  - Introducción a redes neuronales
  - Uso de TensorFlow para construir modelos de clasificación

- **Procesamiento de lenguaje natural (NLP)**
  - Tokenización, extracción de frases nominales
  - Modelos de lenguaje para completar oraciones
  - Uso de transformers y atención (BERT)

- **Juegos con agentes**
  - Algoritmos Minimax y poda alpha-beta
  - Aplicación en juegos como Tic-Tac-Toe
  - Agentes inteligentes para toma de decisiones adversariales

  ## Contacto

  Si quieres conversar sobre este curso o simplemente conectar para compartir ideas sobre las áreas de aplicación, puedes siempre enviarme un mensaje por Linkedin:

  [Linkedin](https://www.linkedin.com/in/ggmarco/)

  También si quieres conocer un poco más sobre mi trabajo personal, puedes visitarme en :

  [SCLFox](https://www.sclfox.com)
  
